  ...
  <div class="site-metric" style="text-align: center;" v-if="siteSetting.theme === 'card'">
    <div
      ...
      <div :class="(site.name === 'total' ? 'highlight-3 ' : 'highlight-2 ') + (isMobile() ? 'item-class-mobile' : 'item-class-pc')">
        ...
        <div class="site-data">
          <span style="font-weight: bold; color: green;"><fa-icon style="width: 16px;" :icon="['fas', 'arrow-up']"></fa-icon></span>
          <span style=""> 上传</span>
          <a-divider type="vertical" />
          <span style="">{{ $formatSize(site.upload) }}</span>
          <br>
          <span style="font-weight: bold; color: red;"><fa-icon style="width: 16px;" :icon="['fas', 'arrow-down']"></fa-icon></span>
          <span style=""> 下载</span>
          <a-divider type="vertical" />
          <span style="">{{ $formatSize(site.download) }}</span>
          <br>
          <span style="font-weight: bold; color: green;"><fa-icon style="width: 16px;" :icon="['fas', 'network-wired']"></fa-icon></span>
          <span style=""> 连接</span>
          <a-divider type="vertical" />
          <span style="">{{ site.seeding }} / {{ site.leeching }}</span>
          <br>
          <span style="font-weight: bold; color: green;"><fa-icon style="width: 16px;" :icon="['fas', 'hard-drive']"></fa-icon></span>
          <span style=""> 做种</span>
          <a-divider type="vertical" />
          <span style="">{{ $formatSize(site.seedingSize) }}</span>
        </div>
        <div class="site-inc-data">
          <span style="font-weight: bold; color: green;"><fa-icon style="width: 12px;" :icon="['fas', 'arrow-up']"></fa-icon></span>
          <span style=""> 昨日上传</span>
          <a-divider type="vertical" />
          <span style="">{{ $formatSize(siteIncrease.yesterday[site.name].upload) }}</span>
          <br>
          <span style="font-weight: bold; color: red;"><fa-icon style="width: 12px;" :icon="['fas', 'arrow-down']"></fa-icon></span>

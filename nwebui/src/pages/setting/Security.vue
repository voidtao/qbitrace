<template>
  <div class="container mx-auto px-4 py-8 max-w-7xl">
    <h1 class="text-2xl font-bold mb-6">安全设置</h1>
    <div class="divider"></div>
    
    <div class="card bg-base-100 shadow-xl">
      <div class="card-body">
        <form @submit.prevent="modify" class="space-y-6">
          <div class="form-control">
            <label class="label">
              <span class="label-text">用户名</span>
            </label>
            <input 
              type="text" 
              v-model="setting.username"
              class="input input-bordered"
              placeholder="登录用户名"
            />
          </div>

          <div class="form-control">
            <label class="label">
              <span class="label-text">密码</span>
            </label>
            <input 
              type="password" 
              v-model="setting.password"
              class="input input-bordered"
              placeholder="登录密码"
            />
          </div>

          <div class="form-control">
            <label class="label">
              <span class="label-text">两步验证密钥</span>
            </label>
            <input 
              type="text" 
              v-model="setting.twoFactorSecret"
              class="input input-bordered"
              placeholder="两步验证密钥"
            />
            <label class="label">
              <span class="label-text-alt">用于生成两步验证码的密钥，留空则禁用两步验证</span>
            </label>
          </div>

          <div class="form-control">
            <label class="label cursor-pointer">
              <span class="label-text">信任所有 SSL 证书</span>
              <input 
                type="checkbox" 
                v-model="setting.trustAllCertificates"
                class="checkbox checkbox-primary"
              />
            </label>
            <label class="label">
              <span class="label-text-alt">信任所有 SSL 证书，请谨慎使用</span>
            </label>
          </div>

          <div class="form-control mt-6">
            <button type="submit" class="btn btn-primary">保存</button>
          </div>
        </form>
      </div>
    </div>
  </div>
</template>

<script setup>
import { ref, onMounted } from 'vue'
import { useToast } from 'vue-toastification'

const toast = useToast()
const setting = ref({
  username: '',
  password: '',
  twoFactorSecret: '',
  trustAllCertificates: false
})

const get = async () => {
  try {
    const response = await fetch('/api/setting/security')
    const data = await response.json()
    setting.value = {
      username: data.username,
      password: '',
      twoFactorSecret: data.twoFactorSecret,
      trustAllCertificates: data.trustAllCertificates
    }
  } catch (error) {
    toast.error(error.message)
  }
}

const modify = async () => {
  try {
    const response = await fetch('/api/setting/security', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
      body: JSON.stringify(setting.value)
    })
    if (!response.ok) throw new Error('保存失败')
    toast.success('修改成功')
    await get()
  } catch (error) {
    toast.error(error.message)
  }
}

onMounted(() => {
  get()
})
</script>

<style scoped>
.container {
  max-width: 1440px;
}
</style>
...
class WatchMod {
  add (options) {
    ...
    watchSet.id = id;
    fs.writeFileSync(path.join(__dirname, '../../storage/data/watch/', id + '.json'), JSON.stringify(watchSet, null, 2));
  ...
  delete (options) {
    fs.unlinkSync(path.join(__dirname, '../../storage/data/watch/', options.id + '.json'));
    fs.unlinkSync(path.join(__dirname, '../../storage/data/watch/set/', options.id + '.json'));
  ...
  modify (options) {
    const watchSet = { ...options };
    fs.writeFileSync(path.join(__dirname, '../../storage/data/watch/', options.id + '.json'), JSON.stringify(watchSet, null, 2));
    if (global.runningWatch[options.id]) global.runningWatch[options.id].destroy();

...
class ScriptMod {
  add (options) {
    ...
    scriptSet.id = id;
    fs.writeFileSync(path.join(__dirname, '../../storage/data/script/', id + '.json'), JSON.stringify(scriptSet, null, 2));
  ...
  delete (options) {
    fs.unlinkSync(path.join(__dirname, '../../storage/data/script/', options.id + '.json'));
  ...
  modify (options) {
    const scriptSet = { ...options };
    fs.writeFileSync(path.join(__dirname, '../../storage/data/script/', options.id + '.json'), JSON.stringify(scriptSet, null, 2));
    if (global.runningScript[options.id]) global.runningScript[options.id].destroy();

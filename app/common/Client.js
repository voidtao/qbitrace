...
class Client {
  ...
  async getMaindata () {
    ...
    try {
      ...
      await util.runRecord('INSERT INTO client_q (time, clientid, upspeed, dlspeed, upcount, dlcount, usedspace, freespace, uploaded, downloaded) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [moment().unix(), this.id, maindata.uploadSpeed, maindata.downloadSpeed, this.maindata.seedingCount, this.maindata.leechingCount, this.maindata.usedSpace, this.maindata.freeSpaceOnDisk, this.maindata.allTimeUpload, this.maindata.allTimeDownload]);
      this.status = true;
      this.errorCount = 0;
    } catch (error) {
